      ...
      <div class="pt-20 container mx-auto px-6 py-24 min-h-[60vh] flex items-center justify-center">
        <div class="text-center max-w-2xl w-full">
          ...
          <div class="grid gap-4 sm:grid-cols-3 max-w-xl mx-auto">
            ${contacts.map(c => `
              <a href="${c.href}"
                ${c.primary ? '' : 'target="_blank" rel="noopener noreferrer"'}
                class="flex flex-col items-center gap-3 rounded-xl border ${c.primary ? 'border-primary bg-primary/5' : 'border-border bg-card'} p-6 text-muted-foreground transition-all hover:border-primary hover:text-primary hover:shadow-lg"
              >
                ${c.icon}
                <span class="text-sm font-semibold text-foreground">${c.label}</span>
                <span class="text-xs text-muted-foreground">${c.value}</span>
              </a>
            `).join('')}
          </div>
        </div>
